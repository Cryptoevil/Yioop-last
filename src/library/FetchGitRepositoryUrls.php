...
class FetchGitRepositoryUrls implements CrawlConstants
{
     ...
     * @param array $site_value contains original Git url crawled
    ...
    public static function setGitRepositoryUrl($url_to_check, $counter, $seeds,
        $repository_indicator, $site_value, $total_git_urls, $all_git_urls)
    {
        $git_internal_urls = [];
        if (!strpos($url_to_check, self::GIT_URL_CONTINUE)) {
            ...
            for ($j = 0; $j < $total_git_urls; $j++) {
                ...
                $seeds[$counter][self::WEIGHT] = $site_value[1];
                $seeds[$counter][self::CRAWL_DELAY] = $site_value[2];
