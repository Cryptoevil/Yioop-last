...
class WebQueueBundle implements Notifier
{
        ...
        $filter_size, $num_urls_ram, $min_or_max)
    {
        ...
        */
        ...
        $adjust_weight_callback = ($min_or_max == CrawlConstants::MIN) ? null :
            C\NS_LIB . "adjustWeightCallback";
        ...
        */
        ...
        if (file_exists($dir_name . "/crawl_delay.ftr")) {
            ...
            $this->crawl_delay_filter =
                new BloomFilterFile($dir_name . "/crawl_delay.ftr",
                $filter_size);
        }
    ...
    {
        $this->to_crawl_queue->normalize($new_total);
    ...
    {
        $time = time();
        if (file_exists($this->dir_name . "/robot_timestamp.txt")) {
