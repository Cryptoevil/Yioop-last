...
class WikiMediaJob extends MediaJob
{
    ...
    public function checkPrerequisites()
    {
        ...
        if ($delta > C\WIKI_UPDATE_INTERVAL) {
            ...
            L\crawlLog("Performing medias podcasts update");
            return true;
        }
    ...
    public function downloadPodcastItem($url, $type = "mp4")
    {
        ...
        if (stripos($data, "#EXT-X-STREAM-INF") !== false) {
            ...
            for ($i = 0; $i  < $num_lines; $i++) {
                if (preg_match('/\s*\#EXT\-X\-STREAM\-INF(.*)/i', $lines[$i],
                    $matches)) {
        ...
        foreach ($playlist_urls as $playlist_url) {
            $file_name = UrlParser::getDocumentFilename($playlist_url) . "." .
                UrlParser::getDocumentType($playlist_url);
        ...
        $title = "", $pubdate = null)
    {
        ...
        $file_name = basename($file_name_parts[0]);
