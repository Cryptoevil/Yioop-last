...
class Tokenizer
{
     ...
     *
     * @param string $string is the parent string
     * @param string $substring is the string checked to be a sub-string
     *      of $string
    ...
    private static function step2()
    {
        ...
        if (!self::$step1_changes) {
            ...
            if ($max_suffix != "" && self::in(self::$rv_string, $max_suffix))
                self::$buffer = substr_replace(self::$buffer,"",
                    self::$rv_start + self::$max_suffix_pos,
                    strlen($max_suffix));
        }
