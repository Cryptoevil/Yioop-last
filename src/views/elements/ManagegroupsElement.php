...
class ManagegroupsElement extends Element
{
    ...
    public function render($data)
    {
        ...
        $data['TABLE_TITLE'] = [
            $base_url => tl('managegroups_element_subscribed_groups'),
            $browse_url => tl('managegroups_element_joinable_groups')
        ];
        if (isset($data['browse']) && $data['browse'] == 'true') {
            $data['TABLE_TITLE_SELECTED'] = $browse_url;
        } else {
            $data['TABLE_TITLE_SELECTED'] = $base_url;
        }
        ...
        <?php
            ...
            foreach ($data['GROUPS'] as $group) {?>
                ...
                foreach ($group as $col_name => $group_column) {
                    ...
                    $td_style = (in_array($data['FORM_TYPE'], ['editgroup',
                        'changeowner', 'graphstats', 'inviteusers',
                        'statistics']) && ($data['FORM_TYPE'] !=
                        'editgroup' || $col_name != 'owner') &&
                        $data['CURRENT_GROUP']['name'] == $group['GROUP_NAME'])?
                    ...
                    if (in_array($col_name, $dropdown_columns)) {
                        ...
                        if (in_array($data['FORM_TYPE'], ['changeowner',
                            'graphstats', 'statistics']) &&
                            $data['CURRENT_GROUP']['name'] ==
                            $group['GROUP_NAME']) {
                            $td_style = " class='admin-edit-row-field' ";
                        }
                        if ($group['GROUP_ID'] != C\PUBLIC_GROUP_ID &&
                            ...
                            $out_url =  ($data['FORM_TYPE'] == 'changeowner'
                                ...
                                $group['GROUP_ID'];
                            e("<td $td_style><b><a href='" . $out_url.
                                "'>$group_column". "</a></b><br />[".
                                tl('managegroups_element_num_users',
                                $group['NUM_MEMBERS'])."]");
                        } else {
                            e("<td $td_style><b>$group_column</b><br />[".
                            tl('managegroups_element_num_users',
                            $group['NUM_MEMBERS'])."]");
                        }
                        if (($is_root ||
                ...
                if ($group['OWNER_ID'] != $_SESSION['USER_ID'] ||
                    ...
                    $group['GROUP_NAME']) {
                    ?>
                    <td <?=$td_style?>><a href="<?= $edit_url . 'group_id='.
                        $group['GROUP_ID'] ?>"><?=
    ...
    public function renderGroupsForm($data)
    {
        ...
        <tr><td colspan="2" class="center"><button class="button-box"<?php
            if ($data['FORM_TYPE'] == 'editgroup') {
                e("id='focus-button'");
            }?>
    ...
    public function renderInviteUsersForm($data)
    {
        ...
        ?>
        <div class='float-opposite'>[<a href='<?= $base_url ?>'>X</a>]</div>
