...
/**
 * Element responsible for drawing the side bar with sign in/create account,
 * search source options, search settings, and tool info for search pages
 *
 * @author Chris Pollett
 */
class SearchoptionsElement extends Element
{
    /**
     * Method responsible for drawing the side bar with more
    ...
    public function render($data)
    {
        ...
        <h2 class="option-heading"><?php
            if ($logged_in) {
                e(tl('searchoptions_element_welcome_user',
                    $_SESSION['USER_NAME']));
            } else {
                e(tl('searchoptions_element_welcome'));
            }
            ?></h2>
            ...
            if (!$logged_in) {
                ...
                tl('searchoptions_element_signin') ?></a></li><?php
            } else {
                ?><li><a href="<?=$admin_url . $token_string ?>"><?=
                tl('searchoptions_element_admin') ?></a></li>
                <li><a href="<?=C\BASE_URL ?>?a=signout"><?=
                    tl('searchoptions_element_signout') ?></a></li><?php
            }
            if ((!$logged_in) &&
                ...
                <li><a href="<?=rtrim($register_url . "a=createAccount&amp;" .
                    $token_string, "?&amp;") ?>"><?=
                    tl('searchoptions_element_create_account')
                    ?></a></li>
                <?php
            }?>
            </ul>
        ...
        <h2 class="option-heading" ><?=
            tl('searchoptions_element_categories')?></h2>
        ...
        <h2 class="option-heading" ><?=
            ...
            if (C\SUBSEARCH_LINK && !empty($data["SUBSEARCH"])) {
                if ($data["SUBSEARCH"] == 'images') { ?>
                    <li><label for="image-size"><b><?=
                        tl('searchoptions_element_size_label')
                        ?></b></label><?php
                        $this->view->helper("options")->render(
                ...
                if ($data["SUBSEARCH"] == 'videos') { ?>
                    <li><label for='min-video-duration'><b><?=
                        tl('searchoptions_element_minduration')
                        ?></b></label><?php
                    ...
                    <li><label for='max-video-duration'><b><?=
                        tl('searchoptions_element_maxduration')
                        ?></b></label><?php
                        $this->view->helper("options")->render(
            ...
            <li><label for="locale"><b><?=
                tl('searchoptions_element_language_label') ?></b></label><?php
                $this->view->element("language")->render($data); ?></li>
            <li><label for="per-page"><b><?=
                tl('searchoptions_element_results_per_page') ?></b></label><?php
                $this->view->helper("options")->render(
                "per-page", "perpage", $data['PER_PAGE'],
                $data['PER_PAGE_SELECTED']); ?></li>
            <li><label for="open-in-tabs"><b><?=
                tl('searchoptions_element_open_in_tabs') ?></b></label>
            ...
            <li><label for="safe-search"><b><?=
                tl('searchoptions_element_safe_search') ?></b></label>
                <input type="checkbox" id="safe-search" name="safe_search"
                    value="true" <?php  if (isset($data['SAFE_SEARCH']) &&
                        $data['SAFE_SEARCH'] == 'true') {
                        ?>checked='checked'<?php
                    } ?> /></li>
            <li class="none"><button type="submit"><?=
                tl('searchoptions_element_save') ?></button></li>
            </ul>
            </form>
        <?php
        $tools = [];
        if (empty($token_string))  {
            ...
            $trending_url = B\controllerUrl('trending', true) . $token_string;
        }
        $tools[$trending_url] = tl('searchoptions_element_trending');
        $tools[$pages_url] = tl('searchoptions_element_wiki_pages');
        if (in_array(C\REGISTRATION_TYPE, ['no_activation',
            'email_registration', 'admin_activation'])) {
            $tools[$suggest_url] = tl('searchoptions_element_suggest');
        }
        if ($tools != []) { ?>
            <h2 id="tools" class="option-heading"><?php
                e(tl('searchoptions_element_tools'))?></h2>
            <ul class="square-list">
            <?php
            ...
            if (!empty(C\SEARCHBAR_PATH)) {
                ?>
                <li><a href="javascript:window.external.AddSearchProvider('<?=
                C\SEARCHBAR_PATH
                    ?>')"><?=tl('searchoptions_install_plugin')
                ?></a></li>
                </ul>
                <?php
            }
