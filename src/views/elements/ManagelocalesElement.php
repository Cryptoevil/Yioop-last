...
class ManagelocalesElement extends Element
{
    ...
    public function render($data)
    {
        ...
        foreach ($data['LOCALES'] as $locale) {
            $td_style = ($data['FORM_TYPE'] == 'editlocale' &&
                $data['CURRENT_LOCALE']['localename'] ==
                $locale['LOCALE_NAME']) ? " class='admin-edit-row' " : "";
            $align_style = (empty($td_style)) ? " class='align-right' " :
                " class='admin-edit-row align-right' ";
            ?>
            <tr><td <?=$td_style ?>><a href='<?=$base_url .
                "&amp;arg=editstrings&amp;selectlocale=".$locale['LOCALE_TAG']
                ?>' ><?= $locale['LOCALE_NAME']?></a></td><?php
            if (!$_SERVER["MOBILE"]) { ?>
                <td <?=$td_style?>><?=$locale['LOCALE_TAG']?></td>
                <td <?=$td_style?>><?=$locale['WRITING_MODE']?></td><?php
                if (empty($td_style)) {
                    $gr_class = ($locale['ACTIVE']) ? " class='green' " :
                        " class='red' ";
                } else {
                    $gr_class = ($locale['ACTIVE']) ?
                        " class='admin-edit-row green' " :
                        " class='admin-edit-row red' ";
                }
                ?>
            ...
            <td <?=$align_style ?>><?=
                $locale['PERCENT_WITH_STRINGS']?></td><?php
            if ($data['FORM_TYPE'] != 'editlocale' ||
                $data['CURRENT_LOCALE']['localename'] !=
                $locale['LOCALE_NAME']) { ?>
                <td <?=$td_style?>><a href='<?=$base_url .
                    "&amp;arg=editlocale&amp;selectlocale=".
                    $locale['LOCALE_TAG']?>' ><?=
                    tl('managelocales_element_edit')?></a></td><?php
            } else { ?>
                <td class='admin-edit-row-field'>
                <a href="<?= $base_url; ?>"><b><?=
                tl('managelocales_element_edit') ?></b></a></td><?php
            } ?>
            <td <?=$td_style?>><a href='<?=$base_url
