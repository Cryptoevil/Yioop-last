...
class SearchController extends Controller implements CrawlConstants
{
    ...
    public function initializeUserAndDefaultActivity(&$data)
    {
        ...
        $data['IMAGE_SIZES'] = [ 'all' => tl('search_controller_allsizes'),
        ...
        if ($activity == "trending") {
            ...
            if(isset($_REQUEST['category']) && in_array($_REQUEST['category'],
                $media_categories)) {
            ...
            foreach ($media_categories as $category) {
                $category_name = $source_model->getSubsearchName(
                    $category, $data['LOCALE_TAG']);
                $category_name = ($category_name == $category) ?
                    ucfirst($category_name) : $category_name;
                $data['MEDIA_CATEGORIES'][$trending_url . "category=" .
                    $category] = $category_name;
            }
            if ($arg == 'chart' && !empty($_REQUEST['term']) &&
                !empty($_REQUEST['period'])) {
                $term = $this->clean($_REQUEST['term'], 'string');
                $periods = [C\ONE_DAY, C\ONE_WEEK, 4 * C\ONE_WEEK];
                $period = in_array($_REQUEST['period'], $periods) ?
                    $_REQUEST['period'] : C\ONE_DAY;
                $data['PERIOD'] = $period;
                $data['TERM'] = $term;
                $data['CHART_DATA'] = $trending_model->termScoresForPeriod(
                    $term, $period, L\getLocaleTag(), $data['CATEGORY']);
                if ($_SERVER["MOBILE"]) {
                    $properties = ["title" => "",
        ...
        $subsearches, &$data)
    {
        ...
        if ($data['IS_LANDING'] && (empty($data["SUBSEARCH"]) ||
            ...
            $num_random_trends = (!empty($_SERVER['MOBILE'])) ?
                6 : 9;
            $data['RANDOM_TRENDS'] = $this->model('trending'
                )->randomTrends(L\getLocaleTag(), "news",
                $num_random_trends);
        }
        $data['SCRIPT'] .= "\nlocal_strings = {'spell':'".
...
EOD;
    ...
    public function calculateControlWords($query, $raw, $is_mix, $index_name)
    {
        ...
        $query = preg_replace('/lang:default-major/', "lang:" .
            $locale_major, $query);
        $query = preg_replace('/lang:default/', "lang:" . $locale, $query);
