...
TokenTool can be used to train some language. In some Asian languages,
such as Chinese, Japanese, there are no spaces between words.
In order to segment terms/words from a sentence, a dictionary with some
statistic is needed.

To make a dictionary for segment the specified language:

php TokenTool.php train <locale> <dictionary_files...>

Here locale is the IANA language tag of the locale to store the results for,
dictionary_files... should be a list of text files
with space or new line between each term.
    ...
    case "train":
        ...
        for($i = 3; $i < count($argv); $i++) {
            if (file_exists($argv[$i])) {
                ...
                echo $argv[$i] . " does not exist in " . $file_path;
                exit();
            }
        }
        $segmenter = new StochasticTermSegmenter($argv[2]);
        if ($segmenter->train($texts)) {
            echo "success";
        } else {
            echo "failed";
        }
